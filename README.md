...
tdPaparazzi {
    ...
    libraryVersion.set("0.5.0")                         // default
...
both target the same directory. Override `inputReportDir` to relocate the per-execution
`td-<timestamp>` folders (e.g. out of the way of another plugin that scans
...
```kotlin
testImplementation("io.github.cdsap:td-paparazzi-ext:0.5.0")
```
